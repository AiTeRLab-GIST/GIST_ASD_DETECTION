from model import CMMOEModel2, CMModel2
...
class LightningModel(pl.LightningModule):
    def __init__(self, bert_name='kykim/bert-kor-base'):
        ...
        # tiny/small model
        print(bert_name)
        self.model = CMModel2(bert_name)
        # self.model = MTLCMModel(bert_name)  
    ...
    def training_step(self, batch, batch_idx):
        ...
        asr_output, nlp_output, asr_embed, nlp_embed = self(x)
        ...
        # total_loss = asr_loss + 0.3*mutual_loss + 0.3*l2_loss + nlp_loss
...
def main(args):
    ...
    for i in range(k_fold):
        ...
        run_name = f"da-cmls-moe-whisper-nomoe-{name}_{i}"
        logger = WandbLogger(
            name=run_name,
